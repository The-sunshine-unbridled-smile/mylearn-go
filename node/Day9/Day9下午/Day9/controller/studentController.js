...
const studentModel = require("../dao/studentDAO");
const studentController = {
    ...
    getAllStudent(req,resp){
      //拿到数据
      //1.当前页面
      console.log(req.params.page) //动态路由上的参数部分，:命名
      //limit currentPage-1*pageCount,pageCount
      let params = [];
      params.push((req.params.page-1)*studentController.pageCount)
      params.push(studentController.pageCount)

      //1.totalCount, 2.所有的学生信息
      studentModel.getTotalCount().
        then(total=>{
          console.log("controller.total",total)
          let result = Math.ceil(total[0].totalcount/studentController.pageCount)
          studentModel.getStudent(params).then(info=>{
            console.log("controller.info",info)
            resp.render("studentlist",{list:info,totalCount:result})
          })
      })


      // let userdata = "张三";
      // let datalist = {stuname:"张三",grade:100,sex:"男"}
      // studentModel.getStudent().then(data=>{
      ...
      // resp.send("<h1>欢迎,张三</h1>")
    },
    getDetail(req,resp){
      //resp.locals
      // resp.locals.username="",
      // resp.locals.test=""
      resp.render("detail",{test:"test值"})
  }
}
